...
angular.module('ngCheckers', [])
  ...
  .controller('checkersCtrl', function($scope, $timeout) { 
    var Blue = "Blue", Lime = "Lime", BOARD_WIDTH = 8,
        selectedSquare = null;
      ...
      for (var i = 0; i < BOARD_WIDTH; i++) {
        ...
        for (var j = 0; j < BOARD_WIDTH; j++) {
          if ( (i === 0 &&  j % 2 === 0) || (i === 1 && j % 2 === 1) ) {
            $scope.board[i][j] = new Piece(Lime, j, i);
    ...
    $scope.setStyling = function(square) {
      if (square.player === Blue)
        return {"backgroundColor": "#1500ff"};
      else if (square.player === Lime)
        return {"backgroundColor":"#00ff15"};
    ...
    function movePiece(square) {
      if (square.isChoice) {
        ...
        $scope.player = $scope.player === Blue ? Lime : Blue;
      }
    }

    function isKing(square) {
      if ($scope.player === Blue) {
        if (square.y === 0) 
          return true;
      } else {
    ...
    function becomeKingAfterJump(x, y){
      return ($scope.player === Blue && y == 1) ||
             ($scope.player === Lime && y == BOARD_WIDTH - 2);
    }
    ...
    function setChoices(x, y, depth, matados, oldX, oldY, isKing) {
      ...
      isKing = 
          isKing || 
          ($scope.player === Blue && y == 0) || 
          ($scope.player === Lime && y == BOARD_WIDTH - 1);
      // Upper Choices
